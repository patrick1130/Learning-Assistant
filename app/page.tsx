    ...
    <div onClick={handleToggle} className={`glass-card p-5 rounded-2xl cursor-pointer transition-all duration-300 hover:-translate-y-1 ${isOpen ? 'ring-2 ring-purple-300 bg-white/90' : 'hover:bg-white/80'}`}>
      ...
      {isOpen && (
        <div className="mt-4 ml-8 pt-3 border-t border-purple-100 text-sm leading-7 text-slate-700 animate-fade-in cursor-default" onClick={e => e.stopPropagation()}>
          {loading ? (
            <div className="flex items-center space-x-2 text-purple-400 py-2"><span className="animate-bounce">🪄</span><span>正在施展魔法...</span></div>
          ) : (
            <>
              {data?.visual_svg && (
                <div className="mb-4 p-4 bg-purple-50/50 rounded-xl border border-purple-100 flex justify-center" dangerouslySetInnerHTML={{ __html: data.visual_svg }} />
              )}
  ...
  return (
    <div className="glass-card p-5 rounded-2xl border-dashed border-2 border-indigo-200">
      ...
      <div className="mt-3">
        ...
        {!loading && expandedData && activeTab === 'detail' && <div className="bg-white/80 p-3 rounded-xl text-sm text-slate-700">{expandedData.detailed_explanation}</div>}
      ...
      <div className="max-w-3xl mx-auto">
          ...
          {!result ? (
            <div className="space-y-6">
              ...
              <div className="flex flex-col gap-4 text-lg text-slate-600 items-center">
                <div className="w-full">我想学习 <input className="glass-input mx-2 px-4 py-2 rounded-xl text-purple-700 font-bold w-full md:w-auto focus:outline-none" value={topic} onChange={e => setTopic(e.target.value)} placeholder="例如：勾股定理" /></div>
                <div className="w-full">是为了做 <input className="glass-input mx-2 px-4 py-2 rounded-xl text-purple-700 font-bold w-full md:w-auto focus:outline-none" value={goal} onChange={e => setGoal(e.target.value)} placeholder="例如：解决几何难题" /></div>
              </div>
        ...
        {result && (
          <div className="space-y-8 pb-20 animate-fade-in-up">
            <section className="glass-card p-2 rounded-3xl overflow-hidden shadow-lg">
               {/* 关键修复：传入 title 属性，确保下载文件名正确 */}
